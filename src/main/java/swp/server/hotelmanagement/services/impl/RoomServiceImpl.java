...
public class RoomServiceImpl implements RoomService {
    ...
    public List<RoomDTO> getAllRooms() {
        try {
            List<RoomEntity> roomEntities = roomRepository.getAllByRentFalse();
            List<RoomDTO> roomDTOS = new ArrayList<>();
            roomEntities.stream().forEach(roomEntity -> {
                RoomDTO roomDTO = mapperUtil.mapRoomEntityToDTo(roomEntity);
    ...
    public RoomDTO getRoomByRoomId(int roomId) {
        try {
            return mapperUtil.mapRoomEntityToDTo(roomRepository.getOne(roomId));
        } catch (Exception e) {
            e.getMessage();
            return null;
        }
    ...
    public RoomDTO updateRoom(RoomDTO roomDTO) {
        try {
            ...
            if (roomEntity != null) {
                roomEntity.setRoomcategoryEntity(mapperUtil.mapCateToEntity(roomDTO.getRoomCategoryDTO()));
    ...
    public RoomDTO createNewRoom(RoomDTO roomDTO) {
        try {
            roomRepository.save(mapperUtil.mapRoomDTOToEntity(roomDTO));
            return roomDTO;
        } catch (Exception e) {
            e.getMessage();
        }
